...
class Daemon_Bootstrap
{
 ...
 public static function init()
 {
  ...
  if (isset(Daemon::$settings['configfile']) && !Daemon::loadConfig(Daemon::$settings['configfile'])) {$error = TRUE;}
  ...
  if (!Daemon_Bootstrap::$pidfile) {Daemon_Bootstrap::$pidfile = Daemon::$settings['pidfile'];}
  ...
  if ($runmode == 'start')
  {
   if ($error === FALSE) {Daemon_Bootstrap::start();}
  }
  ...
  elseif ($runmode == 'configtest')
  {
   ...
   foreach (Daemon::$settings as $name => &$value)
   {
    ...
    if (isset(Daemon::$parsedSettings[$name])) {$row['value'] .= ' ('.Daemon::$parsedSettings[$name].')';}
    $rows[] = $row;
   }
   $term->drawtable($rows);
   echo "\n";
   ...
   if ($ok)
   {
    ...
    while ($r = posix_kill(Daemon_Bootstrap::$pid,SIGTTIN))
    {
     ...
     if ($i == 9)
     {
      ...
      if (!posix_kill(Daemon_Bootstrap::$pid,SIGTTIN)) {echo " Oh, his blood is on my hands :'(";}
      else {echo "ERROR: Process alive. Permissions?";}
      break;
     }
     ++$i;
    }
...
\t--help  -  (This help information.)
  ...
  {
   if ($runmode !== '') {echo 'Unrecognized command: '.$runmode."\n";}
 ...
 public static function stop($mode = 1)
 {
  ...
  if (!$ok) {echo '[STOP] ERROR. It seems that phpDaemon is not running'.(Daemon_Bootstrap::$pid?' (PID '.Daemon_Bootstrap::$pid.')':'').".\n";}
  if ($ok && ($mode > 1))
  {
   $i = 0;
   while ($r = posix_kill(Daemon_Bootstrap::$pid,SIGTTIN))
 ...
 public static function getArgs($args)
 {
  ...
  for($i = 1, $il = sizeof($args); $i < $il; ++$i)
  {
   if (preg_match('~^--(.+)~',$args[$i],$match))
   {
    ...
    if (isset($parts[1])) {$out[$key] = $parts[1];}
    else {$out[$key] = TRUE;}
    $last_arg = $key;
   }
   ...
   elseif ($last_arg !== NULL) {$out[$last_arg] = $args[$i];}
  }
  return $out;
 }
}
