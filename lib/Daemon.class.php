...
class Daemon
{
    ...
    */
    public static function initSettings()
    {
        ...
        Daemon::$settings = array('maxrequests' => '1k', 'maxmemoryusage' => '0b', 'maxidle' => '0s', 'expose' => 1, 'pidfile' =>
            '/var/run/phpd.pid', 'defaultpidfile' => '/var/run/phpd.pid', 'configfile' => Daemon::$dir . '/conf/phpdaemon.conf.php',
            'chroot' => '/', 'cwd' => '.', 'path' => NULL, 'startworkers' => 20, 'minworkers' => 20, 'maxworkers' => 80,
            'minspareworkers' => 20, 'maxspareworkers' => 50, 'obfilterauto' => 1, 'logstorage' =>
            '/var/log/phpdaemon-%DATE=d.m.Y%.log', 'logging' => 1, 'user' => NULL, 'group' => NULL, 'keepalive' => '0s',
            'autoreadbodyfile' => 1, 'ipcwstate' => '/var/run/phpdaemon-wstate.ipc', 'autogc' => '1', 'mpmdelay' => '1s',
            'logsignals' => 0, 'chunksize' => '8k', 'maxconcurrentrequestsperworker' => 1000, 'logreads' => 0, 'autoreload' => '0s',
            'autoreimport' => 0, 'logworkersetstatus' => 0, 'logevents' => 0, 'logqueue' => 0, 'microsleep' => 100000, 'logtostderr' =>
            1, 'workerpriority' => 4, 'masterpriority' => 100, 'throwexceptiononshutdown' => 0, 'locale' => '', 'logerrors' => 1, );
    ...
    */
    public static function outputFilter($s)
    {
        ...
        if (Daemon::$settings['obfilterauto'] && (Daemon::$req !== NULL)) {
    ...
    */
    ...
    {
        static $types = array('txt' => 'text/plain', 'htm' => 'text/html', 'html' => 'text/html', 'php' => 'text/html', 'css' =>
            'text/css', 'js' => 'application/javascript', 'json' => 'application/json', 'xml' => 'application/xml', 'swf' =>
            'application/x-shockwave-flash', 'flv' => 'video/x-flv', // images
            'png' => 'image/png', 'jpe' => 'image/jpeg', 'jpeg' => 'image/jpeg', 'jpg' => 'image/jpeg', 'gif' => 'image/gif', 'bmp' =>
            'image/bmp', 'ico' => 'image/vnd.microsoft.icon', 'tiff' => 'image/tiff', 'tif' => 'image/tiff', 'svg' =>
            'image/svg+xml', 'svgz' => 'image/svg+xml', // archives
            'zip' => 'application/zip', 'rar' => 'application/x-rar-compressed', 'exe' => 'application/x-msdownload', 'msi' =>
            'application/x-msdownload', 'cab' => 'application/vnd.ms-cab-compressed', // audio/video
            'mp3' => 'audio/mpeg', 'qt' => 'video/quicktime', 'mov' => 'video/quicktime', // adobe
            'pdf' => 'application/pdf', 'psd' => 'image/vnd.adobe.photoshop', 'ai' => 'application/postscript', 'eps' =>
            'application/postscript', 'ps' => 'application/postscript', // ms office
            'doc' => 'application/msword', 'rtf' => 'application/rtf', 'xls' => 'application/vnd.ms-excel', 'ppt' =>
            'application/vnd.ms-powerpoint', // open office
    ...
    public static function loadSettings($settings)
    {
        ...
        if (isset($settings['path'])) {
            if (isset(Daemon::$settings['chroot'])) {
                Daemon::$pathReal = realpath(Daemon::$settings['chroot'] . ((substr(Daemon::$settings['chroot'], -1) != '/') ? '/' : '') .
    ...
    */
    public static function parseTime($str)
    {
        ...
        preg_replace_callback('~(\d+)\s*([smhd])\s*|(.+)~i', function ($m)use (&$time)
        {
            ...
            if ($time === FALSE) {
                return; }
            $n = (int)$m[1]; $l = strtolower($m[2]); if ($l === 's') {
                $time += $n; } elseif ($l === 'm') {
            $time += $n * 60; } elseif ($l === 'h') {
        $time += $n * 60 * 60; } elseif ($l === 'd') {
    $time += $n * 60 * 60 * 24; }
}
, $str);
return $time;
...
static $bufsize = 1024;
$offset = 0;
$stat = array('idle' => 0, 'busy' => 0, 'alive' => 0, 'shutdown' => 0, 'preinit' => 0, 'waitinit' => 0, 'init' => 0, );
$c = 0;
...
if (!$shm) {
Daemon::log('Couldn\'t open IPC-' . $name . ' shared memory segment (key=' . $key . ', size=' . $size . ', uid=' .
    posix_getuid() . ').');
exit(0);
}
return $shm;
...
if (Daemon::$settings['logtostderr'] && defined('STDERR')) {
fwrite(STDERR, '[PHPD] ' . $msg . "\n");
}
if (Daemon::$logpointer) {
fwrite(Daemon::$logpointer, '[' . date('D, j M Y H:i:s', $mt[1]) . '.' . sprintf('%06d', $mt[0] * 1000000) . ' ' . date
    ('O') . '] [PHPD] ' . $msg . "\n");
}
}
...
$e = explode('=', $m[1]); if (strtolower($e[0]) == 'date') {
    return date($e[1]); }
return $m[0]; }
, $path);
