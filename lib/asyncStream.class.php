...
class AsyncStream
{
    ...
    public function initStream($readFD = NULL, $writeFD = NULL)
    {
        if (is_string($readFD)) {
            ...
            if ($u['scheme'] === 'unix') {
                ...
                if (!Daemon::$useSockets || ($u['scheme'] === 'tcpstream')) {
    ...
    public function setFD($readFD, $writeFD = NULL)
    {
        ...
        if ($this->readBuf === NULL) {
            ...
            $this->readBuf = event_buffer_new($this->readFD, array($this, 'onReadEvent'), array($this, 'onWriteEvent'), array($this,
                'onReadFailureEvent'), array());
            if (!$this->readBuf) {
        ...
        if ($this->writeBuf === NULL) {
            ...
            $this->writeBuf = event_buffer_new($this->writeFD, NULL, array($this, 'onWriteEvent'), array($this,
                'onWriteFailureEvent'), array());
            if (!$this->writeBuf) {
    ...
    {
        if (Daemon::$settings['logevents'] || 1) {
            Daemon::log(__METHOD__ . '()');
        }
