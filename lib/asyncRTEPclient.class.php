...
class AsyncRTEPclient extends AsyncStream
{
 ...
 public function connect()
 {
  ...
  foreach ($this->servers as &$addr)
  {
   ...
   catch (BadStreamDescriptorException $e) {Daemon::log(get_class($e).': Couldn\'t connect to '.$addr.'.');}
  }
  return FALSE;
 }
 ...
 {
  if (!$this->connect()) {Daemon::log('RTEPClient: Couldn\'t send request.');}
 ...
 public function onEvent($packet)
 {
  if (Daemon::$settings['logevents']) {Daemon::log(__METHOD__.': '.Daemon::var_dump($packet));}
  $event = $packet['event'];
  if (!isset($this->callbacks[$event['name']])) {return FALSE;}
  foreach ($this->callbacks[$event['name']] as $cb) {call_user_func($cb,$event);}
  return TRUE;
 }
