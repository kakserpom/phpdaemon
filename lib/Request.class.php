...
class Request
{
    ...
    */
    ...
    {
        $this->header('Set-Cookie: ' . $name . '=' . rawurlencode($value) . (empty($domain) ? '' : '; Domain=' . $domain) . (empty
            ($maxage) ? '' : '; Max-Age=' . $maxage) . (empty($path) ? '' : '; Path=' . $path) . (!$secure ? '' : '; Secure') . (!$HTTPOnly ?
            '' : '; HttpOnly'), false);
    }
    /* @method header
    ...
    */
    public function header($s, $replace = TRUE, $code = NULL)
    {
        ...
        if (!$replace && isset($this->headers[$k])) {
            return FALSE;
        }
        $this->headers[$k] = $s;
        if ($k === 'CONTENT_LENGTH') {
    ...
    */
    public function parseParams()
    {
        ...
        if (isset($this->attrs->server['REQUEST_METHOD']) && ($this->attrs->server['REQUEST_METHOD'] == 'POST') && isset($this->
            attrs->server['HTTP_CONTENT_TYPE'])) {
            parse_str(strtr($this->attrs->server['HTTP_CONTENT_TYPE'], Request::$hvaltr), $contype);
            if (isset($contype['multipart/form-data']) && (isset($contype['boundary']))) {
    ...
    */
    public function out($s, $flush = TRUE)
    {
        ...
        if (!$this->headers_sent) {
            ...
            if (!Daemon::$compatMode) {
                if (!$this->attrs->chunked && !$this->sendfp) {
                    ...
                    for ($i = 0, $s = sizeof($h); $i < $s; ++$i) {
                        ...
                        if (($e[0] == 'CONTENT_DISPOSITION') && isset($e[1])) {
                            ...
                            if (isset($this->mpartcondisp['filename'])) {
                                ...
                                $this->attrs->files[$this->mpartcondisp['name']] = array('name' => $this->mpartcondisp['filename'], 'type' => '',
                                    'tmp_name' => '', 'error' => UPLOAD_ERR_OK, 'size' => 0, );
                                $tmpdir = ini_get('upload_tmp_dir');
                                if ($tmpdir === FALSE) {
                                    $this->attrs->files[$this->mpartcondisp['name']]['fp'] = FALSE;
                                    $this->attrs->files[$this->mpartcondisp['name']]['error'] = UPLOAD_ERR_NO_TMP_DIR;
                                } else {
                                    $this->attrs->files[$this->mpartcondisp['name']]['fp'] = @fopen($this->attrs->files[$this->mpartcondisp['name']]['tmp_name'] =
                                        tempnam($tmpdir, 'php'), 'w');
            ...
            } elseif (($this->mpartstate === 2) || ($this->mpartstate === 3)) // process the body
            {
                if ((($p = strpos($this->attrs->stdinbuf, $ndl = "\r\n--" . $this->boundary . "\r\n", $this->mpartoffset)) !== FALSE) ||
                    ...
                    if (($this->mpartstate === 2) && isset($this->mpartcondisp['name'])) {
                        $this->attrs->post[$this->mpartcondisp['name']] .= binarySubstr($this->attrs->stdinbuf, $this->mpartoffset, $p - $this->
                            mpartoffset);
                    } elseif (($this->mpartstate === 3) && isset($this->mpartcondisp['filename'])) {
                        if ($this->attrs->files[$this->mpartcondisp['name']]['fp']) {
                            fwrite($this->attrs->files[$this->mpartcondisp['name']]['fp'], binarySubstr($this->attrs->stdinbuf, $this->mpartoffset,
                                $p - $this->mpartoffset));
                        }
                    ...
                    if ($p !== FALSE) {
                        if (($this->mpartstate === 2) && isset($this->mpartcondisp['name'])) {
                            ...
                            if (isset($this->attrs->post['MAX_FILE_SIZE']) && ($this->attrs->post['MAX_FILE_SIZE'] < $this->attrs->files[$this->
                                mpartcondisp['name']]['size'])) {
    ...
    */
    public function stdin($c)
    {
        ...
        if (!isset($this->attrs->server['HTTP_CONTENT_LENGTH']) || ($this->attrs->server['HTTP_CONTENT_LENGTH'] <= $this->attrs->
            stdinlen)) {
    ...
    */
    public function finish($status = 0, $zombie = FALSE)
    {
        ...
        if ((Daemon::$parsedSettings['autogc'] > 0) && (Daemon::$worker->queryCounter > 0) && (Daemon::$worker->queryCounter %
