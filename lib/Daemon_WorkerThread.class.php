...
class Daemon_WorkerThread extends Thread {
    ...
    public function overrideNativeFuncs() {
        if (Daemon::supported(Daemon::SUPPORT_RUNKIT_INTERNAL_MODIFY)) {
            ...
            runkit_function_rename('session_encode', 'session_encode_native');
            runkit_function_rename('session_decode', 'session_decode_native');
            ...
            function session_commit() {
                $_SESSION['_ZEN'] = new ZenApi('dwd');
                $data = session_encode();
                $session_name = ini_get('session.name');
                $session_path = ini_get('session.save_path') ? ini_get('session.save_path') : '/tmp';
                $session_id = $_COOKIE[$session_name];
                if (!preg_match('/[\w\d]+/', $session_id)) {
                    $session_id = $_COOKIE[$session_name] = uniqid();
                }
            ...
            function session_start() {
                ...
                if (preg_match('/[\w\d]+/', $session_id)) {
                    ...
                    if (!file_exists($session_file)) {
                        ...
                        $GLOBALS['_SESSION'] = session_decode(file_get_contents($session_file));
                    }
                } else {
                    $session_id = uniqid();
...
//                                    );
    ...
    public function prepareSystemEnv() {
        ...
        $this->setproctitle(
                ...
                . (Daemon::$config->pidfile->value !== Daemon::$config->defaultpidfile->value ? ' (' . Daemon::$config->pidfile->value . ')' : '')
        );
