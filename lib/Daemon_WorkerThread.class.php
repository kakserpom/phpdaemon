...
class Daemon_WorkerThread extends Thread
{
    ...
    */
    public function run()
    {
        ...
        $this->setStatus(4);
        Thread::setproctitle(Daemon::$runName . ': worker process' . (Daemon::$settings['pidfile'] !== Daemon::$settings['defaultpidfile'] ?
        ...
        if (isset(Daemon::$settings['group'])) {
            if ($sg === FALSE) {
                ...
                Daemon::log('Couldn\'t change group to \'' . Daemon::$settings['group'] . "'. Error (" . ($errno = posix_get_last_error
        ...
        if (isset(Daemon::$settings['user'])) {
            if ($su === FALSE) {
                ...
                Daemon::log('Couldn\'t change user to \'' . Daemon::$settings['user'] . "'. Error (" . ($errno = posix_get_last_error()) .
                    '): ' . posix_strerror($errno));
                exit(0);
            }
        }
