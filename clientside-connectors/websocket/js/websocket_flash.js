if (!("WebSocket" in window)) {
  ...
  WebSocket.__initialize = function() {
    if (!WebSocket__swfLocation) {
      console.error("[WebSocket] set WebSocket__swfLocation to location of WebSocketMain.swf");
      return;
    }
    ...
    swfobject.embedSWF(
    	      WebSocket__swfLocation, "webSocketFlash", "10", "10", "9.0.0",
    	      null, {bridgeName: "webSocket"}, {allowScriptAccess : 'always','enablejs':true, swliveconnect : true}, null,
    	      function(e) {
    	        if (!e.success) console.error("[WebSocket] swfobject.embedSWF failed");
    	        else StreamFlashEmulLoaded = true;
    	      }
    	    );
  ...
  function webSocketLog(message) {
    console.log(getPacketTime() + " " + decodeURIComponent(message));
  }
