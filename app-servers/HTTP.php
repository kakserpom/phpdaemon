...
class HTTP extends AsyncServer
{
    ...
    */
    ...
    {
        $this->poolState[$connId] = array('n' => 0, 'state' => 0, 'addr' => $addr, );
    }
    /* @method requestOut
    ...
    */
    public function endRequest($req, $appStatus, $protoStatus)
    {
        ...
        if ($protoStatus === -1) {
            ...
            if ((!Daemon::$parsedSettings['mod' . $this->modname . 'keepalive']) || (!isset($req->attrs->server['HTTP_CONNECTION'])) ||
    ...
    public function readConn($connId)
    {
        ...
        if ($this->poolState[$connId]['state'] === 1) {
            ...
            if (($p = strpos($req->attrs->inbuf, "\r\n\r\n")) !== FALSE) {
                ...
                if (isset($req->attrs->server['HTTP_CONNECTION']) && ($req->attrs->server['HTTP_CONNECTION'] === 'Upgrade') && isset($req->
                    attrs->server['HTTP_UPGRADE']) && ($req->attrs->server['HTTP_UPGRADE'] === 'WebSocket')) {
                    if ($this->WS) {
                        $this->WS->inheritFromRequest($req, $this);
                        return;
                    }
                } else {
                    $req = Daemon::$appResolver->getRequest($req, $this, isset(Daemon::$settings[$k = 'mod' . $this->modname . 'responder']) ?
                        Daemon::$settings[$k] : NULL);
                }
