...
class MongoNode extends AppInstance
{
    ...
    public function deleteObject($o)
    {
        ...
        $this->cache->get('_id.' . ((string )$o['_id']), function ($m)use ($o)
        {
            if (is_string($m->result)) {
                $m->appInstance->delete($m->result); $m->appInstance->delete('_id.' . $o['_id']); }
        }
        );
    }
    /* @method initSlave
    ...
    */
    public function initSlave($point)
    {
        ...
        {
            $node->cursor = $cursor; $cursor->state = 1; $cursor->lastOpId = NULL; foreach ($cursor->items as $k => &$item) {
                if (Daemon::$settings['logevents']) {
                    Daemon::log(get_class($node) . ': caught oplog-record with ts = (' . Daemon::var_dump($item['ts']) . ')'); }
                $cursor->lastOpId = $item['ts']; if ($item['op'] == 'i') {
                    $node->cacheObject($item['o']); } elseif ($item['op'] == 'd') {
                $node->deleteObject($item['o']); } elseif ($item['op'] == 'u') {
            if (isset($item['b']) && ($item['b'] === FALSE)) {
                $item['o']['_id'] = $item['o2']['_id']; $node->cacheObject($item['o']); }
        else {
...
, array('tailable' => TRUE, 'sort' => array('$natural' => 1), 'snapshot' => 1,
//'where' => array('ts' => array('$gt' => $point),'$where' => '(typeof(this._key) == \'string\') || (typeof(this._ev) == \'string\')'),
'where' => array('ts' => array('$gt' => $point), '$exists' => array('_key' => TRUE)), 'parse_oplog' => TRUE, ));
}
}
...
if (!$this->inited) {
    ...
    {
        $req->inited = FALSE; $e = explode(' ', $answer->result); if (isset($e[1])) {
