...
class WebSocketServer extends AsyncServer
{
    ...
    */
    public function inheritFromRequest($req, $appInstance)
    {
        ...
        $set = event_buffer_set_callback($this->buf[$connId], $this->directReads ? NULL : array($this, 'onReadEvent'), array($this,
    ...
    */
    ...
    {
        Daemon::$parsedSettings['mod' . $this->modname . 'maxallowedpacket'] = Daemon::parseSize(Daemon::$settings['mod' . $this->
            modname . 'maxallowedpacket']);
    }
    /* @method addRoute
    ...
    public function handshake()
    {
        ...
        if ($this->onHandshake()) {
            ...
            $reply = "HTTP/1.1 101 Web Socket Protocol Handshake\r\n" . "Upgrade: WebSocket\r\n" . "Connection: Upgrade\r\n" . ($this->
                secprotocol ? 'Sec-' : '') . 'WebSocket-Origin: ' . $this->server['HTTP_ORIGIN'] . "\r\n" . ($this->secprotocol ? 'Sec-' :
            ...
            if (isset($this->server['HTTP_' . ($this->secprotocol ? 'SEC_' : '') . 'WEBSOCKET_PROTOCOL'])) {
                $reply .= ($this->secprotocol ? 'Sec-' : '') . 'WebSocket-Protocol: ' . $this->server['HTTP_' . ($this->secprotocol ?
                    'SEC_' : '') . 'WEBSOCKET_PROTOCOL'] . "\r\n";
            }
            $reply .= "\r\n";
    ...
    public function stdin($buf)
    {
        ...
        if (!$this->handshaked) {
            ...
            while (($l = $this->gets()) !== FALSE) {
                ...
                if ($l === "\r\n") {
                    if (!isset($this->server['HTTP_CONNECTION']) || ($this->server['HTTP_CONNECTION'] !== 'Upgrade') || !isset($this->
                        server['HTTP_UPGRADE']) || ($this->server['HTTP_UPGRADE'] !== 'WebSocket')) {
                        $this->finish();
                        return;
                    }
        ...
        if ($this->handshaked) {
            ...
            if (strlen($this->buf) >= 8) {
                ...
                $this->resultKey = md5($this->computeKey($this->server['HTTP_SEC_WEBSOCKET_KEY1']) . $this->computeKey($this->server['HTTP_SEC_WEBSOCKET_KEY2']) .
                    $bodyData, TRUE);
