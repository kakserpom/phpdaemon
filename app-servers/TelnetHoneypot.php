...
class TelnetSession extends SocketSession
{
    ...
    */
    ...
    {
        $this->buf .= $buf;
        $finish = (strpos($this->buf, $s = "\xff\xf4\xff\xfd\x06") !== FALSE) || (strpos($this->buf, $s = "\xff\xec") !== FALSE) || (strpos
            ($this->buf, $s = "\x03") !== FALSE) || (strpos($this->buf, $s = "\x04") !== FALSE);
        ...
        if ((strlen($this->buf) > 1024) || $finish) {
            $this->finish();
        }
    }
}
